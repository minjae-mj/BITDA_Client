...
const MyPageProfile = (): JSX.Element => {
  ...
  let submitUserName = async () =>{
    ...
    try{
      let changeUsername = await server.patch('/users/modifyuser',{ userName : changedUsername }, 
      {headers : {
        Authorization : `Bearer ${accessToken}`
      }})
      .then(res => res.data)
  ...
  return (
    <div>
      ...
      <StyleProfileForm>
        ...
        <StyleFormElement>
          <span>닉네임: </span><span>{userName}</span>
          <StyleBtnWithEventBlueSml onClick={()=> setNameModal(pre => !pre)}>닉네임 변경하기</StyleBtnWithEventBlueSml>
          {!nameModal ? <StyleEmptyDiv /> : 
          <div>
            <StyledInputProfile type="text" placeholder="변경 후 닉네임을 입력해주세요." onChange={(e) => setChangedUsername(e.target.value)} />
            <StyleBtnBookmarkSml onClick={submitUserName}>변경 확인</StyleBtnBookmarkSml>
          </div>  
          }
        </StyleFormElement>
        ...
        <StyleFormElement>
          ...
          {!passwordModal ? <StyleEmptyDiv /> : (
            <div>
              <StyledInputProfile type="password" data-type="password" placeholder="현재 비밀번호 8자리" onChange={inputPasswordHandler} />
              <StyledInputProfile type="password" data-type="newPassword" placeholder="새 비밀번호 8자리" onChange={inputPasswordHandler} />
              <StyledInputProfile type="password" data-type="confirmPassword" placeholder="새 비밀번호 8자리 확인" onChange={inputPasswordHandler} />
              <StyleBtnBookmarkSml onClick={submitPassword}>변경 확인</StyleBtnBookmarkSml>
            </div>
          )}
        </StyleFormElement>
